...
def optimize_policy(
    ...
    # Inicjalizacja początkowego rozwiązania (zero)
    initial_solution = np.zeros(dim)
    best_rewards = []

    # Ustawienie CMA-ES (minimalizuje, więc odwrócimy znak)
    ...
    for gen in range(max_gens):
        ...
        fitnesses = [-evaluate_weights(w, env_name, num_episodes=episodes_per_eval) for w in candidates]
        ...
        best_reward = -fitnesses[best_index]
        best_rewards.append(best_reward)

        print(f"Generation {gen}: Best average reward = {best_reward:.2f}")
